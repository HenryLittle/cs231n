...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
          ...
          rnn_out, rnn_lstm_cache = lstm_forward(embeded_caption_in, affine_out, Wx, Wh, b)
        # step 4
        temporal_aff_out, temporal_cache = temporal_affine_forward(rnn_out, W_vocab, b_vocab)
        # step 5
        loss, dout = temporal_softmax_loss(temporal_aff_out, captions_out, mask)
        # BACKWARD PASS
        dout, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dout, temporal_cache)

        if self.cell_type == 'rnn':
          drnn_out, dh0, grads['Wx'], grads['Wh'], grads["b"]= rnn_backward(dout, rnn_cache)
        elif self.cell_type == 'lstm':
          drnn_out, dh0, grads['Wx'], grads['Wh'], grads["b"]= lstm_backward(dout, rnn_lstm_cache)

        grads['W_embed'] = word_embedding_backward(drnn_out, embed_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
          ...
          prev_word_embeded = W_embed[prev_idx]
          # vanilla RNN or with LSTM
          if self.cell_type == 'rnn':
            next_h, _ = rnn_step_forward(prev_word_embeded, prev_h, Wx, Wh, b)
            prev_h = next_h
          elif self.cell_type == 'lstm':
